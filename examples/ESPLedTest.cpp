...
bool brightnessTests2() {
	...
	for (int test_case = 0; test_case < NUM_VALUES; test_case++) {
		test.printResultRange(
			512, ESPLedBrightness::percentToAnalog(test_values[test_case]),
			512);
	}
...
bool brightnessTests3() {
	...
	for (int test_case = 0; test_case < NUM_VALUES; test_case++) {
		...
		test.printResult(PWMRANGE, ESPLedBrightness::percentToAnalog(
									   test_values[test_case]));
	}
...
bool blinkTest2() {
	...
	test.printResult(EXPECTED_HZ,
					 ESPLedInterface::millisecondsToHz(INITIAL_MS));
	test.printResult(INITIAL_MS,
					 ESPLedInterface::hzToMilliseconds(EXPECTED_HZ));
