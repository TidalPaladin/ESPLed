...
void ESPLedInterface::pauseAll() {
	if (isStarted()) _eventChain.stop();
}

void ESPLedInterface::resumeAll() {
	if (!isStarted()) _eventChain.start();
}
...
size_t ESPLedInterface::_addEventEveryLed(unsigned long time_ms,
										  std::function<void(ESPLed *)> f) {
...
size_t ESPLedInterface::_addEvent(unsigned long time_ms,
								  EspEventChain::callback_t function) {
