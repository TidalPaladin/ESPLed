...
ESPPulse &ESPPulse::setRefreshRate(unsigned int hz) {
	if (hz == 0)
		...
		Serial.println("Panic: tried to set refresh rate but the event chain "
					   "was too short");
...
unsigned long ESPPulse::getRefreshRate() const {
	if (_eventChain.numEvents() < 1)
		Serial.println("Panic: tried to get refresh rate but the event chain "
					   "was too short");

	const uint32_t MS_BETWEEN_REFRESHES =
		_eventChain.getTimeOf(__ESP_PULSE_REFRESH_RATE_INDEX__);
	return hzToMilliseconds(MS_BETWEEN_REFRESHES);
}

void ESPPulse::setPulseToPeak() { _theta_rads = PI / 2; }

void ESPPulse::setPulseToTrough() { _theta_rads = -1 * PI / 2; }

float ESPPulse::getDeltaThetaFromPeriod(unsigned long period_ms) const {
	if (period_ms == 0) panic();
...
unsigned long ESPPulse::getPeriodFromDeltaTheta(float delta_theta_rads) const {
	...
	if (delta_theta_rads == 0) panic();

	return round(TWO_PI / delta_theta_rads *
				 hzToMilliseconds(getRefreshRate()));
}

void ESPPulse::construct(unsigned long refresh_rate_hz,
						 unsigned long period_ms) {

	if (refresh_rate_hz == 0 || period_ms == 0) panic();
	...
	*/
	...
	const uint8_t SECOND_EVENT_TIME_MS =
		0; // Loop through all Leds immediately after advancing theta
...
float ESPPulse::_sin(float theta) {
	return theta * (1.27323954 + .405284735 * theta *
									 (theta < 0 ? 1.00000000 : -1.00000000));
}
