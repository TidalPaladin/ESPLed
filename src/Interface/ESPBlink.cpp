...
unsigned long ESPBlink::getInterval() const {
	return _eventChain.getTimeOf(INTERVAL_INDEX);
}
unsigned long ESPBlink::getDuration() const {
	return _eventChain.getTimeOf(DURATION_INDEX);
}
...
void ESPBlink::construct(unsigned long interval_ms, unsigned long duration_ms) {
	...
	*/
	...
	_addEventEveryLed(interval_ms, [](ESPLed *led) { led->on(); });

	_addEventEveryLed(duration_ms, [](ESPLed *led) { led->off(); });
}
